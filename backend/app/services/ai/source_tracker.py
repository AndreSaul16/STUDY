...
class Source:
    ...
    kind: str
    """"scripture" | "article" | "search" | "daily" | "mcp"."""
    label: str
    citation: str = ""
    url: str = ""
...
class SourceTracker:
    ...
    def _extract(
        ...
        if tool_name == "buscar_en_biblioteca":
    ...
    def _from_search(result: Dict[str, Any]) -> List[Source]:
        ...
        for item in raw[:_MAX_SEARCH_SOURCES]:
            ...
            label = citation or _text(entry.get("publication"))
            if not label:
                continue
            sources.append(
                Source(
    ...
    def summary(tool_name: str, result: Any) -> str:
        ...
        if tool_name == "buscar_en_biblioteca":
            ...
            return f"{total} resultado{'s' if total != 1 else ''}"
